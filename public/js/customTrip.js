...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('/api/bookings/trip-options', { credentials: 'include' })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            DESTINATIONS = data.data.destinations.map(function(d) { return { id: d.optId, name: d.name, price: d.price }; });
            ACC_TYPES    = data.data.accommodations.map(function(a) { return { id: a.optId, name: a.name, price: a.price }; });
            ROOM_TYPES   = data.data.rooms.map(function(r)         { return { id: r.optId, name: r.name, mult: r.mult }; });
            initWizard();
        })
        .catch(function() { initWizard(); });
});
...
async function submitTrip() {
    ...
    const destNames = selectedDests.map(function(id) {
        var d = DESTINATIONS.find(function(dest) { return dest.id === id; });
        return d ? d.name : id;
    }).join(", ");

    const btn = document.querySelector('.step-nav button:last-child');
    ...
    try {
        ...
        window.location.href = '/booking/travellers?draftId=' + data.data.draftId;
    } catch (err) {
        showError('review-error', err.message || 'Something went wrong. Please try again.');
        if (btn) { btn.disabled = false; btn.textContent = 'Confirm Your Purchase'; }
    }
}
